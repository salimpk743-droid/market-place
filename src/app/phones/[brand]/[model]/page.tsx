...
export default async function ModelPage({ params }: Props) {
  ...
  const result = await searchPhoneSeoListings({ brand: b.name, model: modelName });
  if (!result.total) {
    return <EmptyState title={`${b.name} ${modelName} is not currently listed`} body="Model pages are generated from the site's supported catalog and become indexable when real seller inventory exists." actionHref={`/phones/${b.slug}`} actionLabel={`Browse ${b.name} phones`} />;
  }
